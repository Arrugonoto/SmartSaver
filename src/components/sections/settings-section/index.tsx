...
export const UserSettingsSection = ({
  ...
  return (
    <>
      <section
        className={`flex relative flex-col bg-content1 h-full px-4 py-2 gap-2 rounded-lg ${
          user.name === 'Guest' ? 'pointer-events-none select-none' : ''
        }`}
        tabIndex={-1}
      >
      ...
      {user.name === 'Guest' && (
        <div className="absolute left-2 w-[calc(100%-1rem)] h-[calc(100%-1rem)] bg-black/50 z-[1000] rounded-lg cursor-not-allowed" />
      )}
    </>
  );
};
